...
class NextStep:
    ...
    def test(self,):
        ...
        ***Helper Variables***
        ...
        os.makedirs(results_dir, exist_ok=True)  # 디렉토리 없으면 생성

        # 파일 이름과 경로 설정
        file_name = f"{model_type}_{self.eventlog}"
        output_file = os.path.join(results_dir, f'1hotnext_activity_and_time_{file_name}')
        
        # make predictions
        with open(output_file, 'w', encoding="utf-8") as csvfile:
            spamwriter = csv.writer(csvfile, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
            spamwriter.writerow(["CaseID", "Prefix length", "Ground truth", "Predicted", "Confidence",
                                "Levenshtein", "Damerau", "Jaccard", "Ground truth times",
                                "Predicted times", "RMSE", "MAE"])

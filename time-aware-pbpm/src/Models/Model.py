...
class CSModel:
    ...
    def train(self,
        ...
        early_stopping = keras.callbacks.EarlyStopping(monitor='val_loss',
                                                       patience=25)
        base_path = os.path.abspath(os.path.join(os.path.curdir, ".."))
        model_dir = os.path.join(base_path, 'Model', '1.CS')
        os.makedirs(model_dir, exist_ok=True)
        filepath = os.path.join(model_dir,
                                f"CS_lr_{hparams[HP_LEARNING_RATE]}_units_{hparams[HP_NUM_UNITS]}_DO_{hparams[HP_DROPOUT]}_{self.name}_{{epoch:02d}}-{{val_loss:.2f}}.h5")

        print(f"Model will be saved at: {filepath}")
...
class ALL_TLSTM_Model:
    ...
    def train(self,
        ...
        early_stopping = keras.callbacks.EarlyStopping(monitor='val_loss',
                                                       patience=25)
        base_path = os.path.abspath(os.path.join(os.path.curdir, ".."))
        model_dir = os.path.join(base_path, 'Model','2.TLSTM')
        os.makedirs(model_dir, exist_ok=True)
        filepath = os.path.join(model_dir,
                                f"TLSTM_lr_{hparams[HP_LEARNING_RATE]}_units_{hparams[HP_NUM_UNITS]}_DO_{hparams[HP_DROPOUT]}_{self.name}_{{epoch:02d}}-{{val_loss:.2f}}.h5")
...
class CS_TLSTM_Model:
    ...
    def train(self,
        ...
        early_stopping = keras.callbacks.EarlyStopping(monitor='val_loss',
                                                       patience=25)
        base_path = os.path.abspath(os.path.join(os.path.curdir, ".."))
        model_dir = os.path.join(base_path, 'Model','3.CS_TLSTM')
        os.makedirs(model_dir, exist_ok=True)
        filepath = os.path.join(model_dir,
                                f"CS_TLSTM_lr_{hparams[HP_LEARNING_RATE]}_units_{hparams[HP_NUM_UNITS]}_DO_{hparams[HP_DROPOUT]}_{self.name}_{{epoch:02d}}-{{val_loss:.2f}}.h5")
